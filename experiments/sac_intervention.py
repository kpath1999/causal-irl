...
def init(task='pushing', num_envs=1, log_dir='test', vis=False, train=True):
    print("\n====== Initializing SAC Model with Interventions ======")
    print(f"• Policy network: MLP [256, 256] with Tanh activation")
    print(f"• Using 1 environment (SAC is off-policy and doesn't need VecEnv)")
    print(f"• Replay buffer size: 1e5")
    print(f"• Batch size: 64")
    print(f"• Learning starts after: 1000 steps")
    print(f"• Target entropy: auto")
    ...
    policy_kwargs = dict(layers=[256, 256], act_fun=tf.nn.tanh)
...
def train(task, save_dir, total_timesteps=1e10, save_freq=500, n_envs=1):
    assert n_envs == 1, "SAC in SB2 does not support multiple environments (VecEnv). Please set num_envs=1."
...
def eval(load_dir):
    ...
    print(f"• Final Mean Reward: {df['ep_rewmean'].iloc[-1]:.2f}")
    print(f"• Max Reward Achieved: {df['ep_rewmean'].max():.2f}")
    print(f"• Min Reward Achieved: {df['ep_rewmean'].min():.2f}")
    
    # Plotting reward metrics
    plt.figure(figsize=(12, 6))
    plt.plot(df['total timesteps'], df['ep_rewmean'], label='Mean Reward')
    
    # If ep_reward_std available, fill; otherwise skip
    if 'ep_reward_std' in df.columns:
        plt.fill_between(df['total_timesteps'], 
                         df['ep_rewmean'] - df['ep_reward_std'],
                         df['ep_rewmean'] + df['ep_reward_std'],
                         alpha=0.2, label='±1 Std Dev')
    
    plt.xlabel('Timesteps')
    plt.ylabel('Reward')
    plt.title(f'Training Progress - SAC Vanilla')
    plt.legend()
    plt.grid(True)
...
def main(argv):
    ...
    # save_dir = f'sac_intervention_{FLAGS.task}_{FLAGS.num_envs}_mva_block_catching_1e-3_50k_{timestamp}'
    save_dir = f'sac_vanilla_second_run_success'
