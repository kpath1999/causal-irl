...
def init(task, algorithm_settings, policy_kwargs, task_settings,
    ...
    env = SubprocVecEnv([
        _make_env(task, task_settings, world_settings, log_dir=log_dir, vis=vis) for i in range(num_envs)])
    model = SAC(MlpPolicy,
                env,
...
def train(algorithm_config, task_config, world_config, network_settings, task='pushing', n_envs=10, seed=0):
    ...
    #     seed=0
    #     )
    # # eval_env = MovingAverageActionEnvWrapper(eval_env)

    # eval_callback = EvalCallback(
    #     eval_env, best_model_save_path=os.path.join(save_dir, './logs/best_model'),
    #     log_path=os.path.join(save_dir, './logs/results'), eval_freq=max(save_freq // n_envs, 1))
...
def gen_eval(load_dir):
    ...
    stable_baselines_policy_path = os.path.join(load_dir, 'model')
